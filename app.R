...
header <- dashboardHeader(title = "LAPOP Dashboard", titleWidth = 300)
...
body <- dashboardBody(
    tabItems(
        tabItem(tabName = "overview",
                ...
                fluidRow(
                    tabBox(title="Histogram of Respondents' age structure",
                           tabPanel("Overall",  plotOutput("overview_hist")),
                           tabPanel("Gender",  plotOutput("overview_hist_gender"))
                    ),
                    tabBox(title="Density of Respondents' age structure",
        ...
        tabItem(tabName = "basic",
                ...
                fluidRow(
                    ...
                    box(title = "Boxplot of Answer by Gender",
...
server <- function(input, output) {
    ...
    output$sidebar <- renderMenu({
        sidebarMenu(
            ...
            menuItem("Respondent Overview", tabName = "overview", icon = icon("dashboard")),
    ...
    output$numberBox <- renderValueBox({
        ...
        valueBox(nrow(df), "Respondents", icon = icon("users"),color = "purple")
    })
    ...
    output$femalepercentBox <- renderValueBox({
        ...
        valueBox(paste0(round(nrow(df_female)/nrow(df)*100,digits = 2), "%"), 
                 "Female Ratio", 
    ...
    output$ageBox <- renderValueBox({
        ...
        valueBox(round(mean(df$age), digits = 2), "Average age", icon = icon("list"),color = "purple")
    })
    ...
    output$overview_density <- renderPlot({
        data %>% 
            filter(year %in% input$year,
                   country %in% input$country) %>% 
            ggplot()+
            geom_density(aes(x=age), fill = "#1C6771", color =  "#1C6771",alpha=0.5)+
            theme_classic()+  
            theme(legend.position = "top")
    })
    ...
    output$overview_density_gender <- renderPlot({
        data %>% 
            filter(year %in% input$year,
                   country %in% input$country) %>% 
            ggplot()+
            geom_density(aes(x=age,fill=gender, alpha=0.5, color=gender))+
            theme_classic()+  
            theme(legend.position = "top")
    })
    ...
    output$overview_gender_year <- renderPlot({
        data %>% 
            ...
            ggplot()+
            geom_line(aes(x=year, y=female_ratio),size=1.5,color="#1C6771")+
            geom_point(aes(x=year, y=female_ratio),size=2,color="#1C6771")+
    ...
    output$basic_scaleBox <- renderValueBox({
        ...
        df <- data %>% 
            filter(year %in% input$year,
                   country %in% input$country) %>% 
            select(question1)
        valueBox(paste0(min(unique(as.matrix(df))), " - ",max(unique(as.matrix(df)))), "Answer Scale", icon = icon("list-ol"),color = "purple")
    })
    ...
    output$basic_meanBox <- renderValueBox({
        ...
        valueBox(round(mean(as.matrix(df)), digits = 2), "Average answer", icon = icon("list"),color = "yellow")
    })
    ...
    output$basic_modeBox <- renderValueBox({
        ...
        valueBox(mode, "Mode answer", icon = icon("sort-amount-down"), color = "purple")
    })
    ...
    output$basic_scorehist <- renderPlot({
        ...
        data %>% 
            ...
            theme_classic()+
            labs(title = paste0("Distribution for Answer of ", question1), x="Answer")+
            theme(legend.position = "top")
    })
    ...
    output$basic_scorehist_gender <- renderPlot({
        ...
        data %>% 
            ...
            summarise(number = n()) %>% 
            ggplot() +
            geom_col(aes(x = !!as.symbol(question1), y=number,fill = gender), width = .7,position = "dodge") + 
            theme_classic()+
            labs(title = paste0("Distribution for Answer of ", question1), x="Answer")+
            theme(legend.position = "top")
    })
    ...
    output$basic_agedist <- renderPlot({
        ...
        data %>% 
            ...
            ggplot() +
            geom_col(aes(x = age, y= mean_score), width = 1, fill = "#1C6771") + 
            # geom_smooth(aes(x = age, y= mean_score), se = FALSE, color = "lightblue", lwd = 2)+
            theme_classic()+
            labs(title = paste0("Answer of ", question1, " by Age"), x="Age")
    })
    ...
    output$basic_agedist_gender <- renderPlot({
        ...
        data %>% 
            ...
            summarise(mean_score = mean(!!as.symbol(question1))) %>% 
            ggplot() +
            geom_col(aes(x = age, y= mean_score, fill = gender), width = 1, position = "dodge") + 
            # geom_smooth(aes(x = age, y= mean_score), se = FALSE, color = "lightblue", lwd = 2)+
            theme_classic()+
            labs(title = paste0("Answer of ", question1, " by Age"), x="Age")+
            theme(legend.position = "top")
    ...
    output$basic_genderscore <- renderPlot({
        ...
        data %>% 
            filter(year %in% input$year,
                   country %in% input$country) %>% 
            ggplot() +
            geom_boxplot(aes(x = gender, y= !!as.symbol(question1), fill = gender, color=gender),lwd = 1,alpha =0.7)+
            theme_classic()+
            labs(title = paste0("Answer of ", question1, " by Gender"), x="Gender", y = "Answer")+
            theme(legend.position = "top")
    })
    ...
    output$time_year_mean_score <- renderPlot({
        ...
        data %>% 
            ...
            summarise(mean_score = mean(!!as.symbol(question1))) %>% 
            ggplot() +
            geom_point(aes(x = year, y= mean_score), size = 2, color = "#1C6771") +
            geom_line(aes(x = year, y= mean_score), size = 1.5, color = "#1C6771") + 
            # geom_smooth(aes(x = year, y= mean_score), se = FALSE, color = "lightblue", lwd = 2)+
            theme_classic()+
            labs(title = paste0("Answer of ", question1, " by Year"), x="Year")+
            theme(legend.position = "top")
    })
    ...
    output$time_year_mean_score_gender <- renderPlot({
        ...
        data %>% 
            ...
            summarise(mean_score = mean(!!as.symbol(question1))) %>% 
            ggplot() +
            geom_point(aes(x = year, y= mean_score, color=gender,), size = 2) +
            geom_line(aes(x = year, y= mean_score, color=gender,), size = 1.5) + 
            theme_classic()+
            labs(title = paste0("Answer of ", question1, " by Year(Gender)"), x="Year")+
            theme(legend.position = "top")
    })
    ...
    output$time_year_boxplot <- renderPlot({
        ...
        data %>% 
            ...
            geom_boxplot(aes(x = as.factor(year),
                         ...
                         color="#1C6771") + 
            theme_classic()+
            labs(title = paste0("Boxplot of Answer of ", question1, " by Year"), x="Year")+
            theme(legend.position = "top")
    })
    ...
    output$cross_scaleBox <- renderValueBox({
        ...
        df <- data %>% 
            filter(year %in% input$year,
                   country %in% input$country) %>% 
            select(question2)
        valueBox(paste0(min(unique(as.matrix(df))), " - ",max(unique(as.matrix(df)))), "Answer Scale of Cross Question", icon = icon("list-ol"),color = "purple")
    })
    ...
    output$cross_meanBox <- renderValueBox({
        ...
        valueBox(round(mean(as.matrix(df)), digits = 2), "Average answer of Cross Question", icon = icon("list"),color = "yellow")
    })
    ...
    output$cross_corBox <- renderValueBox({
        ...
        valueBox(round(cor, digits = 2), "Correlation", icon = icon("sync-alt"), color = "purple")
    })
    ...
    output$cross_cross_mean_score_gender <- renderPlot({
        ...
        data %>% 
            ...
            summarise(mean_cross_variable = mean(!!as.symbol(question2))) %>% 
            ggplot()+
            geom_col(aes(x=as.factor(!!as.symbol(question1)), y=mean_cross_variable, fill=gender),
                     width = .5,
                     position = "dodge")+
            theme_classic()+
            labs(x=paste0("Mean of", question1), y=question2)+
    ...
    output$cross_boxplot <- renderPlot({
        ...
        data %>% 
            filter(year %in% input$year,
                   country %in% input$country) %>% 
            ggplot() +
            geom_boxplot(aes(x = as.factor(!!as.symbol(question1)),
