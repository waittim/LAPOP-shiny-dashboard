...
server <- function(input, output) {
    ...
    output$ageBox <- renderValueBox({
        ...
        valueBox(round(mean(df$age), digits = 2), "Average age", icon = icon("list"),color = "olive")
    })
    ...
    output$basic_meanBox <- renderValueBox({
        ...
        valueBox(round(mean(as.matrix(df)), digits = 2), "Average answer", icon = icon("list"),color = "olive")
    })
    ...
    output$basic_modeBox <- renderValueBox({
        ...
        valueBox(mode, "Mode answer", icon = icon("sort-amount-down"), color = "olive")
    })
    ...
    output$cross_scaleBox <- renderValueBox({
        ...
        df <- data %>% 
            filter(year %in% input$year,
                   country %in% input$country) %>% 
            select(question2)
        valueBox(paste0(min(unique(as.matrix(df))), " - ",max(unique(as.matrix(df)))), "Answer Scale of Cross Question", icon = icon("list-ol"),color = "olive")
    })
    ...
    output$cross_meanBox <- renderValueBox({
        ...
        valueBox(round(mean(as.matrix(df)), digits = 2), "Average answer of Cross Question", icon = icon("list"),color = "olive")
    })
    ...
    output$cross_corBox <- renderValueBox({
        ...
        valueBox(round(cor, digits = 2), "Correlation", icon = icon("sync-alt"), color = "olive")
    })
