...
#names(data) <- c("id","country","year","gender","age","Democracy is Better than Any Other Form of Government","Satisfaction with Democracy", "Perception of Democracy")
data$id <- seq(1, N, 1)
...
body <- dashboardBody(tabItems(
    tabItem(
        ...
        fluidRow(
            box(
                title = "Respondents' overall age structure",
                br(),
                "Histogram",
                plotOutput("overview_hist"),
                solidHeader = TRUE,
                collapsible = TRUE
            ),
            box(
    ...
    tabItem(
        ...
        fluidRow(
            box(
                ...
                plotOutput("basic_scorehist"),
                solidHeader = TRUE,
                collapsible = TRUE
            ),
            box(
                title = "Distribution of Answers by Gender",
                plotOutput("basic_scorehist_gender"),
                solidHeader = TRUE,
                collapsible = TRUE
            ),
            box(
                title = "Average Answer for each age ",
                plotOutput("basic_agedist"),
                solidHeader = TRUE,
                collapsible = TRUE
            ),
            box(
    ...
    tabItem(tabName = "timeseries",
            fluidRow(
                box(
                    plotOutput("time_year_mean_score"),
                    solidHeader = TRUE,
                    collapsible = TRUE
                ),
                box(
                    plotOutput("time_year_mean_score_gender"),
                    solidHeader = TRUE,
                    collapsible = TRUE
                ),
                box(
                    plotOutput("time_year_boxplot"),
                    solidHeader = TRUE,
                    collapsible = TRUE
                ),
                box(
    ...
    tabItem(
        ...
        fluidRow(
            box(
                plotOutput("cross_cross_mean_score"),
                solidHeader = TRUE,
                collapsible = TRUE
            ),
            box(
                plotOutput("cross_boxplot"),
                solidHeader = TRUE,
                collapsible = TRUE
            ),
            box(
